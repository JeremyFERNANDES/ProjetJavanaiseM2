...
public class JvnServerImpl extends UnicastRemoteObject implements JvnLocalServer, JvnRemoteServer {
	...
	public void suppressionElementCache() throws JvnException {
		...
		while (it.hasNext()) {
			...
			if ((currentLock == lockStates.NL || currentLock == lockStates.RC || currentLock == lockStates.WC)
					&& idObjASupprimmer == -1) {
				idObjASupprimmer = (Integer) cle;
			} else if (currentLock == lockStates.NL
					&& (toRemoveLock == lockStates.RC || toRemoveLock == lockStates.WC)) {
				idObjASupprimmer = (Integer) cle;
			} else if (currentLock == lockStates.NL && toRemoveLock == lockStates.NL
		...
		if (idObjASupprimmer == -1) {
			...
			if (this.cache.get(idObjASupprimmer).getVerrou() == lockStates.WC
					|| this.cache.get(idObjASupprimmer).getVerrou() == lockStates.RC) {
				try {
	...
	public Hashtable<String, Integer> getHashmap() {
		return new Hashtable<String, Integer>(correspondanceNomId);
	}
